...
const UserInfo = new Command("userinfo", "UserInfo", {slash:false,prefix:true});
...
const Names = ["Информация о пользователе","Уровень","Кол-во опыта","Блоков","Монеток","Кол-во блоков за 1 сообщение","Уровень кирки","Бонусы","Предупреждения","Простые бонусы","Экстра-бонусов"];
...
const filterData = (obj) => {
    ...
    if(typeof obj === "object" && !values.includes(obj)){
        ...
        return keys.map((v,i)=>`${Names[Names.length-2+i]}: ${vals[i]}`).join(", ");
    }
    return obj
}
...
UserInfo.setSharedThread((bot)=>{
    ...
    bot.on("messageCreate", async(message)=>{
        if(message.author.bot || message.content.startsWith(bot.config.bot.prefix)) return;
        if(message.channel.type == discord.ChannelType.DM);
        const uid = message.author.id;
        if(!config.db[uid]) config.db[uid] = userInfoExample(message.author.username);
...
UserInfo.setPrefixAction((m,b)=>{
    ...
    m.channel.send(Object.keys(data).map((v,i)=>Names[i]+": "+filterData(data[v])).join("\n")+`\nДо следующего уровня: ${data.level*5}`);
    UserInfo.writeConfig(config);
})
