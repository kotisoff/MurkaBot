...
const convert = new Command("convertblocks","",{prefix:true, slash:false});
...
convert.setPrefixAction(async(m,b)=>{
    ...
    if(profile.blocks/10>=coinsToConvert&&coinsToConvert>=0){
        ...
        m.channel.send(`Конвертация завершена успешно. У вас осталось ${profile.blocks} блоков.`);
    }
    else m.channel.send("Недостаточно блоков для конвертации. Конвертация отменена.");
})
