...
const BuyPickaxe = new Command("upgradepick", "", {
  slash: false,
  prefix: true
});
...
BuyPickaxe.setPrefixAction(async (m, b) => {
  ...
  if (profile.coins < cost || profile.pickaxelvl < 1)
    return m.channel.send(
      "Недостаточно монеток для улучшения кирки, либо кирка отсутствует"
    );
  profile.coins -= cost;
  profile.picklvl += 1;
  profile.blockpermsg = profile.picklvl + Math.floor(profile.picklvl * 0.125);
  m.channel.send(
    `Вы потратили ${cost} монеток для улучшения кирки. Уровень кирки теперь равен ${profile.picklvl}, у вас осталось ${profile.coins} монеток.`
  );
